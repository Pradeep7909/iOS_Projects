...
class ChatViewController: UIViewController , UIScrollViewDelegate{
    ...
    var currentUser = CurrentUserManager.shared.currentUser
    ...
    @IBAction func msgSendTapped(_ sender: Any) {
        if let messageText = msgTextField.text {
            ...
            Analytics.logEvent("message_sent", parameters: [
                "user_name": self.currentUser?.userName ?? "Arfof"
            ])
    ...
    //screen setup
    func setScreen(){
        currentUser = CurrentUserManager.shared.currentUser
